...
/*
/!\ Une collision existe entre une "zone" et un "probe". 
Soit une planète et un vaisseau : 
	* Le vaisseau est un probe, 
	* la planète est une zone.
La collision est détectée par le système de trigger, qui publie un événement ON_ENTER ou ON_EXIT selon le cas. 
Le probe peut être détruit ou désactivé, ce qui déclenche un ON_EXIT implicite pour toutes les zones qu'il chevauchait.
Les zones ne se chevauchent jamais entre elles, et les probes ne se chevauchent jamais entre eux. 
Le système de trigger ne gère pas les collisions physiques, seulement les événements de chevauchement.
*/
