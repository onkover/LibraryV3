...
namespace LibV3
{
    struct AABB3d {
        ...
        LV2_FORCEINLINE bool IsValid() const noexcept { return min.x <= max.x; }
        LV2_FORCEINLINE Vec3f Center() const noexcept { return (min+max)*0.5f; }
        LV2_FORCEINLINE Vec3f Extent() const noexcept { return (max-min)*0.5f; }

        LV2_FORCEINLINE void Expand(const Vec3f& p) noexcept {
            if (p.x<min.x) min.x=p.x; if (p.y<min.y) min.y=p.y; if (p.z<min.z) min.z=p.z;
            if (p.x>max.x) max.x=p.x; if (p.y>max.y) max.y=p.y; if (p.z>max.z) max.z=p.z;
        }
        LV2_FORCEINLINE bool Contains(const Vec3f& p) const noexcept {
            return p.x>=min.x&&p.x<=max.x&&p.y>=min.y&&p.y<=max.y&&p.z>=min.z&&p.z<=max.z;
        }
        LV2_FORCEINLINE bool Intersects(const AABB3d& b) const noexcept {
            return max.x>=b.min.x&&min.x<=b.max.x&&max.y>=b.min.y&&min.y<=b.max.y&&max.z>=b.min.z&&min.z<=b.max.z;
        }
        LV2_FORCEINLINE AABB3d Union(const AABB3d& b) const noexcept {
            return {
